...
//        int i = names.stream().reduce(0, (a, b) -> a+b.length());
    ...
    public static void jobSequence(){
        // Given an 2D array Jobs of size Nx3, where Jobs[i][0] represents JobID , Jobs[i][1] represents Deadline , Jobs[i][2] represents Profit associated with that job. Each Job takes 1 unit of time to complete and only one job can be scheduled at a time. The profit associated with a job is earned only if it is completed by its deadline. Find the number of jobs and maximum profit.
        // int[][] arr = new int[][]{{1, 4, 20} , {2, 1, 10} , {3, 1, 40} , {4, 1, 30}};
        int[][] arr = new int[][]{{1, 2, 100} , {2, 1, 19} , {3, 2, 27} , {4, 1, 25} , {5, 1, 15}};
        Map<Integer, List<int[]>> groupedMap = Arrays.stream(arr).collect(groupingBy(ele -> ele[1]));
        int sum = 0;
        System.out.println(groupedMap);
        for(Map.Entry<Integer, List<int[]>> entry: groupedMap.entrySet()){
            if(entry.getValue().size() > 1){
                int[] ar = entry.getValue().stream().sorted((e1, e2) -> e1[2]-e2[2]).reduce((a,b) -> b).get();
                System.out.println(arr[2]);
                sum+= ar[2];
            }
            else if (entry.getValue().size() == 1) sum+=entry.getValue().get(0)[2];
        }
